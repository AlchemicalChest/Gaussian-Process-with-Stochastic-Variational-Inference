...
class GPClassificationSVI(StochasticVariationalInference):
    ...
    def objective(self, z, x_tr, y_tr):
        ...
        u = [np.array(self.parameters[M*C+M*M*j:M*C+M*M*(j+1)]).reshape(M,M).dot(z) \
        ...
        for i in range(N):
            ...
            for k in range(S):
                f = [samples[k]*sqrt(sigma[i]**2) + mu[j][i][0] for j in range(C)]
                exp_sum = sum([mpmath.exp(f[j]) for j in range(C)])
                sample_sum += float(mpmath.log(exp_sum)*weights[k])
            sample_sum /= sqrt(math.pi)
            
            val += mu[y][i][0]-sample_sum
            
            mean = self.parameters[M*y:M*(y+1)]
            L  = np.tril(self.parameters[M*C+M*M*y:M*C+M*M*(y+1)].reshape(M,M))
            cov  = L.dot(L.T)
            pu = 1
            for j in range(M):
                pu *= multivariate_normal.pdf(u[y][j], mean[j], cov[j,j], True)
            val *= pu
        val += np.log(abs(linalg.det(L)))
        return val
    ...
    def gradient(self, z, x_tr, y_tr):
        ...
        for i in range(N):
            ...
            for k in range(S):    
                ...
                sample_sum += weights[k] * mpmath.exp(f[y])/exp_sum 
            sample_sum /= sqrt(math.pi)
...
#        self.hidden_variables = x_tr[np.random.choice(range(N), size=M), :]
#        if self.objective(z, x_tr, y_tr) < val:
        ...
        for i in range(N):
            ...
            for j in range(C):
                for k in range(S):    
                    ...
                    sample_sum += weights[k] * mpmath.exp(f[j])/exp_sum 
                sample_sum /= sqrt(math.pi)
